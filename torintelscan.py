#!/usr/bin/env python3
# torscanner1.py - TorIntel Scan (full) with queue/evidence flags
# Single-file scanner: recon, fingerprinting, header leak detection,
# SSRF/LFI (opt-in), port scanning over Tor, BTC recon, clearnet enrichment,
# mirror detection, evidence capture (Playwright + WARC), crawler/discovery.
#
...
# Load spaCy model (ensure `python -m spacy download en_core_web_sm` has been run)
try:
...
# Optional WARC support (graceful if unavailable)
try:
...
# Optional Playwright (graceful if unavailable)
try:
...
# ---------------- Misconfiguration & Deanonymization Flags ---------------
...
# ---------------- Evidence capture (Playwright + WARC) -------------------
...
def evidence_capture(url, case_dir, warc_path=None, tor_socks="127.0.0.1:9050", user_agent=None,
    ...
    """Queue-aware evidentiary capture.
    - wait_selector: CSS selector to wait for before screenshot (e.g., main content)
    - wait_keywords: list of keywords that indicate the *page you want* (if found, stop waiting)
    - max_wait: max seconds to wait for queue/challenge to pass
    - timeline: if True, save periodic screenshots every snapshot_every seconds (queue progression)
    - newnym: if True, request NEWNYM between retries via Tor control port
    """
...
def crawl_onions(seeds, max_depth, delay, output_file, active_tests=False, evidence=None, hibp_key=None, pdns_provider=None, pdns_key=None, mirror_index=None, ev_opts=None):
    ...
    while q:
        ...
        if evidence:
            ev = evidence_capture(
                url,
                evidence.get("case_dir"),
                warc_path=evidence.get("warc_path"),
                wait_selector=ev_opts.get("wait_selector"),
                wait_keywords=ev_opts.get("wait_keywords"),
                max_wait=ev_opts.get("max_wait", 120),
                timeline=ev_opts.get("timeline", False),
                snapshot_every=ev_opts.get("snapshot_every", 5),
                newnym=ev_opts.get("newnym", False),
                control_port=ev_opts.get("control_port", 9051),
                control_password=ev_opts.get("control_password", None)
            )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="torscanner1 - Scan/crawl .onion sites for intel and evidentiary capture.")
    ...
    # Queue / waiting options (Playwright)
    parser.add_argument("--queue-wait", type=int, default=0, help="Extra wait between queue checks (not used directly; use --max-wait)")
    parser.add_argument("--queue-log", default=None, help="Path to write queue progress log (audit JSONL)")
    parser.add_argument("--wait-selector", default=None, help="CSS selector to wait for (e.g., 'main,#content')")
    parser.add_argument("--wait-keywords", default=None, help="Comma-separated keywords to wait for in page content")
    parser.add_argument("--max-wait", type=int, default=120, help="Max wait for queue/challenge in seconds")
    parser.add_argument("--timeline", action="store_true", help="Save timeline screenshots while waiting")
    parser.add_argument("--snapshot-every", type=int, default=5, help="Seconds between timeline snapshots")
    parser.add_argument("--newnym", action="store_true", help="Request NEWNYM during waits (requires stem and Tor control)")
    parser.add_argument("--control-port", type=int, default=9051, help="Tor control port for NEWNYM")
    parser.add_argument("--control-password", default=None, help="Tor control password (if set)")
    # CLEARNET enrichment & PDNS
    ...
    # note: minor compatibility for flag name used earlier
    pdns_provider = args.pdns_provider if (getattr(args, 'enrich_clearnet', False) or args.pdns_provider) else None
    pdns_key = args.pdns_key if args.pdns_provider else None
    mirror_index = args.index_file if args.mirror_detect else None

    # Prepare evidence options for Playwright capture
    ev_opts = {
        "wait_selector": args.wait_selector,
        "wait_keywords": [k.strip() for k in args.wait_keywords.split(",")] if getattr(args, 'wait_keywords', None) else None,
    ...
    if args.crawl:
        ...
        new_ones = crawl_onions(
            seeds, args.max_depth, args.delay, args.output,
            active_tests=args.active_tests, evidence=evidence,
            hibp_key=hibp_key, pdns_provider=pdns_provider, pdns_key=pdns_key,
            mirror_index=mirror_index, ev_opts=ev_opts
        )
        if new_ones:
